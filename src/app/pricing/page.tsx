  ...
  return (
    <>
      ...
      <section className="section-pad bg-white">
        <div className="container-site mb-10 rounded-2xl border border-black/[0.08] bg-[#fafafa] px-6 py-5 text-center sm:px-10">
        ...
        <div className="container-site grid gap-6 lg:grid-cols-3">
          {programmes.map((p) => (
            <article
              key={p.id}
              id={p.id}
              className="flex flex-col rounded-2xl border border-black/[0.08] bg-[#fafafa] p-6 sm:p-8"
            >
              <p className="text-[0.6875rem] font-semibold uppercase tracking-[0.14em] text-muted">
                {p.ageLabel}
              </p>
              ...
              <div className="mt-6 flex flex-col gap-2">
                <button
                  ...
                  className="rounded-full bg-ink px-4 py-2.5 text-sm font-semibold text-white hover:bg-ink-soft"
                >
        ...
        <div className="container-site mt-12 max-w-2xl">
          <p className="text-sm leading-relaxed text-muted">
            Production payments use <strong className="text-ink">Paystack</strong>{" "
            }in <strong className="text-ink">USD</strong> (${COURSE_PRICE_USD} =
            600 cents). Set keys in{" "}
            <code className="text-ink">.env.local</code>. Until then, use demo
            access to explore the LMS.
          </p>
          <div className="mt-6">
