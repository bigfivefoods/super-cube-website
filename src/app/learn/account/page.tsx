...
function AccountPageInner() {
  ...
  async function enableReminders() {
    if (!("Notification" in window)) {
      setMsg("Notifications not supported.");
      return;
    }
    ...
    saveLmsState(next);
    setState(next);
    setMsg(perm === "granted" ? "Reminders enabled." : "Notifications denied.");
  }
  ...
  const growth =
    pre && post
      ...
      : null;
  const comparison = pre ? compareAttempts(pre.result, post?.result) : null;
