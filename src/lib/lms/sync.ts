...
export function mergeLmsStates(
  ...
  // facePulses: union by date, prefer newer `at`
  type Pulse = NonNullable<LocalLmsState["facePulses"]>[number];
  const pulseByDate = new Map<string, Pulse>();
  for (const p of [...(a.facePulses || []), ...(b.facePulses || [])]) {
    const prev = pulseByDate.get(p.date);
    if (!prev || Date.parse(p.at) >= Date.parse(prev.at)) {
      pulseByDate.set(p.date, p);
    }
  }
  const facePulses = Array.from(pulseByDate.values())
  ...
  return {
    ...
    certificateEarnedAt:
      a.certificateEarnedAt && b.certificateEarnedAt
        ...
        : a.certificateEarnedAt || b.certificateEarnedAt,
    facePulses,
    orgCode: newer.orgCode || older.orgCode,
